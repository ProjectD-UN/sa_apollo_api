...
const url$2 = process.env.CENTERS_URL || '35.227.44.194';
const port_centers = process.env.CENTERS_PORT || '3000';
...
const httpLink = apolloLinkHttp.createHttpLink({uri: '/graphql', fetch});
const formatResponse = new apolloLink.ApolloLink((operation, forward) => {
	return forward(operation).map(response => {
		if(response.data.getLabs){
			console.log("Aca estamos");
			var aux = response.data.getLabs;
			return aux
		}
	})
});

const link = formatResponse.concat(httpLink);
